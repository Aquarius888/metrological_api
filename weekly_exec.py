...
def get_list_of_urls():
    ...
    for n in xrange(len(widgets_json['widgets'])):
        # metrological bullshit
        if 'realtime' not in widgets_json['widgets'][n]['tags'] \
            and 'last60mins' not in widgets_json['widgets'][n]['createOptions']['timespan'] \
            and 'lastweek' in widgets_json['widgets'][n]['createOptions']['timespan']:
...
def to_graphite(data, prefix, country_acr, metric_name, applid):
    ...
    if country_acr in data["data"] \
        and 'categories' in data["data"][country_acr] \
        ...
        if len(data_type) < len(categories):
            for n in xrange(len(data_type)):
                ...
                for i in xrange(len(categories)):
                    ...
                    if data_type_data[i] is None:
                        continue
                    message = '{0}.{1}.{2}.{3}'.format(prefix, country_acr, metric_name, name_data)
                    print(message, data_type_data[i], tmp)
                    # graphitesend.send(
                    #    metric=message,
                    #    value=data_type_data[i],
                    #    timestamp=tmp,
                    # )
        ...
        for number, data_s in data["data"].items():
            ...
            for i in xrange(len(categories)):
                if data_s.get(country_acr).get('data')[i] is None:
                    continue
                tmp = categories[i] / 1000 + settings.TIME_SHIFT
                message = '{0}.{1}.{2}.{3}'.format(prefix, country_acr, metric_name, number)
                print(message, data_s.get(country_acr).get('data')[i], tmp)
                # graphitesend.send(
                #    metric=message,
                #    value=data_s.get(country_acr).get('data')[i],
                #    timestamp=tmp,
                # )


if __name__ == "__main__":
    netflix_metrics.configure_logging(settings.loglevel).info('RUN WEEKLY.........')
    list_of_urls = get_list_of_urls()
    send_data(list_of_urls)
    netflix_metrics.configure_logging(settings.loglevel).info('FINISH WEEKLY.........')
